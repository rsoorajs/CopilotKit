...
The local `agentId` is what `useAgent`, state-manager subscriptions, and `onAgentsChanged` see. The `remoteAgentId` is invisible to subscribers — it only changes the URL path the proxy hits (`/agent/<remoteAgentId>/run`).
...
<PropertyReference name="params" type="CopilotKitCoreRegisterProxiedAgentParams">
  ...
  <PropertyReference name="remoteAgentId" type="string" required>
