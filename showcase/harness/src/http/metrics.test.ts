...
describe("metrics registry", () => {
  it("exposes the five baseline counters as TYPE-annotated Prometheus text", () => {
    ...
    expect(text).toContain("# TYPE showcase_harness_probe_runs counter");
    expect(text).toContain('showcase_harness_probe_runs{dimension="smoke"} 2');
    expect(text).toContain('showcase_harness_probe_runs{dimension="health"} 1');
    expect(text).toContain("# TYPE showcase_harness_alert_matches counter");
    expect(text).toContain(
      'showcase_harness_alert_matches{rule="smoke-red-tick"} 1',
    );
    expect(text).toContain("# TYPE showcase_harness_alert_sends counter");
    expect(text).toContain("# TYPE showcase_harness_rule_reloads counter");
    // Anchor to line-start/end so `rule_reloads 1` isn't satisfied by e.g.
    // `rule_reloads 10`. Same rationale for hmac_failures below.
    expect(text).toMatch(/^showcase_harness_rule_reloads\s+1$/m);
    expect(text).toContain("# TYPE showcase_harness_hmac_failures counter");
    expect(text).toMatch(/^showcase_harness_hmac_failures\s+1$/m);
  });
  ...
  it("emits HELP lines with a description for every metric", () => {
    ...
    const text = renderPrometheus(reg);
    expect(text).toMatch(/^# HELP showcase_harness_probe_runs .+/m);
    expect(text).toMatch(/^# HELP showcase_harness_alert_matches .+/m);
    expect(text).toMatch(/^# HELP showcase_harness_alert_sends .+/m);
    expect(text).toMatch(/^# HELP showcase_harness_rule_reloads .+/m);
    expect(text).toMatch(/^# HELP showcase_harness_hmac_failures .+/m);
    expect(text).toMatch(/^# HELP showcase_harness_probe_duration_ms .+/m);
  });
  ...
  it("records histogram observations for probe latency", () => {
    ...
    expect(text).toMatch(
      /showcase_harness_probe_duration_ms_bucket\{dimension="smoke",le="1000"\}\s+2/,
    );
    expect(text).toMatch(
      /showcase_harness_probe_duration_ms_bucket\{dimension="smoke",le="\+Inf"\}\s+3/,
    );
    expect(text).toMatch(
      /showcase_harness_probe_duration_ms_count\{dimension="smoke"\}\s+3/,
    );
  });
  ...
  it("escapes \\r and \\n in label values", () => {
    ...
    const text = renderPrometheus(reg);
    expect(text).toContain(
      'showcase_harness_probe_runs{dimension="line1\\r\\nline2"}',
    );
  });
  ...
  it("emits a zero-valued empty histogram with full bucket schema before any observation", () => {
    ...
    for (const bound of ["10", "50", "100", "500", "1000", "5000", "+Inf"]) {
      expect(text).toContain(
        `showcase_harness_probe_duration_ms_bucket{le="${bound}"} 0`,
      );
    }
    expect(text).toMatch(/^showcase_harness_probe_duration_ms_sum 0$/m);
    expect(text).toMatch(/^showcase_harness_probe_duration_ms_count 0$/m);
  });
  ...
  it("merges `le` into existing labels for populated histograms (alphabetical sort — locked)", () => {
    ...
    expect(text).toMatch(
      /showcase_harness_probe_duration_ms_bucket\{dimension="smoke",key="a",le="10"\}\s+1/,
    );
  });
  ...
  it("emits mixed label sets consistently across empty and populated series", () => {
    ...
    const text = renderPrometheus(reg);
    expect(text).toMatch(
      /showcase_harness_probe_duration_ms_sum\{dimension="smoke"\}\s+50/,
    );
    expect(text).not.toMatch(/^showcase_harness_probe_duration_ms_sum 0$/m);
  });
});
