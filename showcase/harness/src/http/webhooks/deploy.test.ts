...
describe("POST /webhooks/deploy", () => {
  ...
  it("rejects payload missing required fields with 400 including zod-flatten detail", async () => {
    ...
    // without reading the harness service log.
...
describe("POST /webhooks/deploy — webhook_rejections metric wiring", () => {
  ...
  it("increments webhook_rejections on bad signature", async () => {
    ...
    const text = renderPrometheus(metrics);
    expect(text).toMatch(
      /showcase_harness_webhook_rejections\{reason="bad-signature"\}\s+1/,
    );
    expect(text).toMatch(
      /showcase_harness_hmac_failures\{reason="bad-signature"\}\s+1/,
    );
  });
  ...
  it("increments webhook_rejections on missing timestamp (split-reason: missing-timestamp)", async () => {
    ...
    const text = renderPrometheus(metrics);
    expect(text).toMatch(
      /showcase_harness_webhook_rejections\{reason="missing-timestamp"\}\s+1/,
    );
    expect(text).toMatch(
      /showcase_harness_hmac_failures\{reason="missing-timestamp"\}\s+1/,
    );
  });
  ...
  it("increments webhook_rejections on missing-headers (both absent) with combined reason", async () => {
    ...
    const text = renderPrometheus(metrics);
    expect(text).toMatch(
      /showcase_harness_webhook_rejections\{reason="missing-headers"\}\s+1/,
    );
    expect(text).toMatch(
      /showcase_harness_hmac_failures\{reason="missing-headers"\}\s+1/,
    );
  });
  ...
  it("increments webhook_rejections on missing signature (split-reason: missing-signature)", async () => {
    ...
    const text = renderPrometheus(metrics);
    expect(text).toMatch(
      /showcase_harness_webhook_rejections\{reason="missing-signature"\}\s+1/,
    );
    // Deprecated alias still populated for HMAC-category reasons —
    // `missing-signature` is in HMAC_REASONS.
    expect(text).toMatch(
      /showcase_harness_hmac_failures\{reason="missing-signature"\}\s+1/,
    );
  });
  ...
  it("increments webhook_rejections with reason=invalid-json on bad body (NOT mirrored to hmac_failures)", async () => {
    ...
    const text = renderPrometheus(metrics);
    expect(text).toMatch(
      /showcase_harness_webhook_rejections\{reason="invalid-json"\}\s+1/,
    );
    ...
    expect(text).not.toMatch(
      /showcase_harness_hmac_failures\{reason="invalid-json"\}/,
    );
  });
  ...
  it("increments webhook_rejections with reason=invalid-payload when schema rejects", async () => {
    ...
    const text = renderPrometheus(metrics);
    expect(text).toMatch(
      /showcase_harness_webhook_rejections\{reason="invalid-payload"\}\s+1/,
    );
  });
});
