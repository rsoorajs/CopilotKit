...
If you'd rather have a streaming progressive render rather than a one-shot validated render, see the sibling page [BYOC — Hashbrown](/byoc-hashbrown) for the same scenario with `@hashbrownai/react`.
...
## Not supported on Spring AI

Spring AI's manifest marks `byoc-json-render` as `not_supported`. The Java backend doesn't expose the streaming structured-output channel that `@json-render/react` expects on the frontend. Use [BYOC — Hashbrown](/byoc-hashbrown) on Spring AI instead, or pick a different framework if json-render is required.

## Comparing the two BYOC patterns

Both `byoc-json-render` and [`byoc-hashbrown`](/byoc-hashbrown) solve the same problem with two different rendering libraries. The agent contract is similar; the React glue, validation strategy, and rendering behaviour differ.
