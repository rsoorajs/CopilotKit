...
If you'd rather work with an explicit catalog of registered components rather than streaming a JSON tree, see the sibling page [BYOC — JSON Render](/byoc-json-render) for the same scenario implemented with `@json-render/react`.
...
Both `byoc-hashbrown` and [`byoc-json-render`](/byoc-json-render) solve the same problem (agent-authored structured UI, rendered through a typed catalog), with two different rendering libraries. Pick whichever you already use elsewhere — the agent contract is the same shape; the React glue is what changes.
