...
CopilotKit doesn't lock you into one agent framework. The runtime talks to your agent over **[AG-UI](/concepts/ag-ui-protocol)**, an open, event-driven protocol that standardizes how agents communicate with applications:
...
7. Threads, persistence, and realtime sync (when configured) are mediated by the [Intelligence Platform](/concepts/intelligence-platform) — the Enterprise backend that sits beside the runtime.

## Where to go next

- **Practical setup** — [Quickstart](/quickstart) walks through wiring all three layers in ~10 minutes against the Built-in Agent.
- **Protocol depth** — [AG-UI documentation](/concepts/ag-ui-protocol) covers every event type, transport option, and middleware hook.
...
- **Enterprise deployment** — [Intelligence Platform](/concepts/intelligence-platform) covers Threads, Persistence, Observability, and the hosted-vs-self-hosted decision.
