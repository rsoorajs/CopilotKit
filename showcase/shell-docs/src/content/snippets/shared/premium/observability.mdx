...
<OpsPlatformCTA
  variant="card"
  title="Observability requires an Enterprise Intelligence Platform license key"
  body="Sign up for a free account to get a public license key and start monitoring your agents."
...
<Callout type="info">
  All observability hooks require a `publicLicenseKey` or `publicAPIkey` - Get
  yours free at [https://cloud.copilotkit.ai](https://cloud.copilotkit.ai)
</Callout>
...
<Callout type="warn">
  **Important:** Observability hooks will **not trigger** without a valid key.
  This is a security feature to ensure observability hooks only work in
