...
<OpsPlatformCTA
  variant="card"
  title="Headless UI requires an Enterprise Intelligence Platform license key"
...
CopilotKit offers _fully headless UI_ through the `useCopilotChatHeadless_c` hook. By using this hook, you can build your own chat interfaces
...
export const Chat = () => {
  ...
  const { suggestions, generateSuggestions, sendMessage } =
    useCopilotChatHeadless_c(); // [!code highlight]
...
```tsx title="src/app/components/chat.tsx"
import { useFrontendTool, useCopilotChatHeadless_c } from "@copilotkit/react-core";
