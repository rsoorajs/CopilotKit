...
//
// Backend: the agent behind this endpoint is the shared CrewAI crew via
// HttpAgent — voice is an input-modality concern, not an agent-behavior
// concern.
//
...
//    instance directly. V1's constructor `Omit`s `transcriptionService` from
//    the forwarded options, so setting it as a V1 constructor arg is a no-op.
//    Until the upstream V1 wrapper is taught to forward transcription, per-
//    demo routes that need it must reach through `.instance`.
//
...
//    affordance visible while failing loudly with a 4xx (AUTH_FAILED → 401)
//    instead of a silent 503 when the deployment is misconfigured.
//
// References:
...
const agents: Record<string, AbstractAgent> = {
  ...
  // default-agent lookups resolve against the same crew.
  default: createAgent(),
};
