 ...
 *
 * The agent writes these via its `set_notes` tool. The Strands ToolBehavior
 * for that tool has `state_from_args=notes_state_from_args`, which emits a
 * `StateSnapshotEvent` the moment the tool fires — so the UI re-renders
 * before the surrounding text response finishes streaming.
 *
