...
// Headless Interrupt demo (Built-in Agent port).
//
...
// The built-in agent (TanStack AI) has no interrupt primitive, so we instead
// register `schedule_meeting` as a frontend tool and gate the UI on whether
// the tool is currently awaiting a user decision. The async handler returns a
// Promise that only resolves when the user interacts with the external popup
...
function Layout() {
  ...
  useFrontendTool({
    ...
    // popup. This is the built-in agent shim for the LangGraph headless
