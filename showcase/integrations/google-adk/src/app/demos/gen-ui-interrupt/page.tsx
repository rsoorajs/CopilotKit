...
// Gen UI Interrupt demo (Google ADK port).
//
...
// hook. Google ADK does NOT have an equivalent interrupt primitive, so we
// adapt the demo by registering a frontend tool with `useFrontendTool`. The
...
export default function GenUiInterruptDemo() {
  return (
    <CopilotKit runtimeUrl="/api/copilotkit" agent="gen_ui_interrupt">
...
function Chat() {
  ...
  // prop when the user clicks a slot or cancels. This is the ADK
  ...
  useFrontendTool({
    ...
    // acts on the picker. This is the Google ADK shim for
  ...
  return (
    <CopilotChat agentId="gen_ui_interrupt" className="h-full rounded-2xl" />
  );
}
