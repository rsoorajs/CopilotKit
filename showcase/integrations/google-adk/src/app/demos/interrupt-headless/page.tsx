...
// Headless Interrupt demo (Google ADK port).
//
...
// Google ADK has no interrupt primitive, so we instead register
// `schedule_meeting` as a frontend tool and gate the UI on whether the tool
// is currently awaiting a user decision. The async handler returns a Promise
// that only resolves when the user interacts with the external popup —
...
export default function InterruptHeadlessDemo() {
  return (
    <CopilotKit runtimeUrl="/api/copilotkit" agent="interrupt_headless">
      <Layout />
    </CopilotKit>
  );
}
...
function Layout() {
  ...
  useFrontendTool({
    ...
    // popup. This is the Google ADK shim for the LangGraph headless
  ...
  return (
    <div className="grid h-screen grid-cols-[1fr_420px] bg-[#FAFAFC]">
      ...
      <div className="border-l border-[#DBDBE5] bg-white">
        <CopilotChat agentId="interrupt_headless" className="h-full" />
      </div>
    </div>
  );
}
