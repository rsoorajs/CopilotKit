 ...
 *
 * The agent writes these via its `set_notes` tool, which returns a
 * `state_update(...)` so the AG-UI emitter pushes a deterministic
 * `StateSnapshotEvent` after the tool call. The UI reflects every update
 * in real time because the parent page subscribes via
