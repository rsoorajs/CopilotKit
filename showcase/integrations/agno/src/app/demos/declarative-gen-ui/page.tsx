...
/**
 * Declarative Generative UI (A2UI — Dynamic Schema) demo (Agno).
 *
 * Pattern:
 ...
 *      configured with `injectA2UITool: false` — the backend Agno agent
 *      (`src/agents/a2ui_dynamic_agent.py`) owns the `generate_a2ui` tool
 *      explicitly. The A2UI middleware still serialises the registered
 *      catalog schema into `state.copilotkit.context` so the secondary
 *      LLM inside `generate_a2ui` knows which components are available.
 *
 * Reference:
